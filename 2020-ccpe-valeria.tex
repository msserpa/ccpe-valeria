...
\title{Understanding Memory Prefetcher Performance over Parallel Applications: From Real to Simulated}
...
\ctitle{Understanding Memory Prefetcher Performance over Parallel Applications: From Real to Simulated}, \cjournal{Concurrency and Computation: Practice and Experience}, \cvol{TBD}.}
...
Studies about the memory hierarchy in HPC systems require parallel architecture simulators that consider the interactions that occur among the different cores. %\fbm{the phrase above, this one, and the following one sound redundant. Remove one?}
...
This benchmark comprises ten applications, each of which encapsulates a certain type of computation that is often processed by HPC applications. 
We used nine of the ten NPB applications, namely:
...
At the light of these results, it may be advisable to set the L2 prefetcher on its own, as opposed to the L1+L2 setting, since the lack of transparency of the L1 and L2 prefetcher, the L1 line fill buffer contention, and the additional energy consumption of the L1 prefetcher may not outweigh its increase in performance.%\ms{cuidado com esse tipo de frase... tem que ter base boa} 
\fbm{literally no one cares about prefetcher "transparency".}\vsg{arguable, as I mentioned above} \fbm{ok, simulator people do, good point, but people who will actually use the machine do not. The way the paragraph is written just seems too connected to the idea of performance, that I did not (and the reviewers will not) consider "transparency" to be of any relevance. I think we should talk more about L1 line fill buffer contention}%\vsg{Also, transparency is an ambiguous term here: the whole memory hierarchy is transparent, since the processor/programmer don't even know that there are caches or prefetchers}\vsg{solved}
...
In this subsection, we aim to detail the CG executions to better understand the CG results, showing how the application behavior contributes to the different observations. %\fbm{"attain.." might leave a bad impression. Maybe "detail the execution of CG"?}
...
% However, there is a \textcolor{red}{\textbf{Tony?}} stark contrast between configurations with L2 prefetcher, and those without. \textcolor{red}{\textbf{aqui ainda estamos falando da figura ~\ref{fig:HITM_CG}?}}
% Configurations with the L2 prefetcher do not have an IPC increase, as the L2 prefetcher is correctly speculating and bringing the data to the caches even with a low core count.
% The IPC tendency of configurations with an L2 prefetcher is to decrease, as the prefetcher effectively hid the latency of DRAM accesses, but increasing the number of cores generated contention in the DRAM access.
% On the other hand, configurations without the L2 prefetcher cannot hide this latency, and their IPC increases as we are able to retain larger portions of the working set in cache.
...
However, the memory requirements of different HPC applications may vary substantially, requiring different execution parameters. %\fbm{arguable, one could say EP isn't HPC}\vsg{yes, but I'm not simply talking about EP, in fact, I'm considering all types of HPC applications and their variable communication/memory requirements}
...
\vsg{resposta ao ponto ii do revisor 2}
...
%não usar system-wide perf (exemplo: l2-pf-rfo-hitm-other-core de qualquer benchmark para 1 thread demonstra várias requisições em situação impossível)
%Um ponto bastante ressaltado ao longo do trabalho é a dificuldade de se trabalhar com o sistema de memória como um todo, mas, principalmente com o prefetcher.
%A obscuridade e confidencialidade das implementações de hardware real torna difícil a implementação desses módulos em simuladores, bem como definir os parâmetros de simlação.
...
%Com o aumento do paralelismo é natural a diminuição do tempo de execução total da aplicação.
With the increase in application parallelism, the execution time naturally becomes smaller. 
%Entretanto, na Section~\ref{subsubsec:real_ipc}, vemos que o desempenho por core vai diminuindo conforme aumenta-se o número de cores utilizados.
...
%Tal resultado evidencia o impacto que acessos à memória tanto em busca de dados quando para comunicação possuem sobre o desempenho da aplicação.
%Quanto mais comunicação é realizada, maior a contenção em recursos compartilhados (rede de interconexão e cache L3 compartilhada entre todos os cores).
...
%Dessa forma, chegamos a um ponto em que o prefetcher não consegue mitigar suficientemente a latência da memória, resulting in small IPC values.
...
%Portanto, analisar o padrão de comunicação das aplicações pode ser uma tarefa importante para melhorar o desempenho das mesmas, uma vez que para execuções com um alto nível de paralelismo o prefetcher começa a perder a eficácia.
