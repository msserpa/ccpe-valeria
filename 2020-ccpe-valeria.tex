...
\title{\new{Investigating} Memory Prefetcher Performance over Parallel Applications: From Real to Simulated}
...
\abstract[Summary]{
\new{Memory prefetcher algorithms are widely used in processors to mitigate the performance gap between the processors and the memory subsystem. 
...
In this paper, we contribute to shed light on the memory prefetcher's role in the performance of parallel HPC applications, considering the prefetcher algorithms offered by both the real hardware and the simulators.} 
...
\begin{table}[!htb]
    \centering
    \footnotesize
    \caption{\new{Configuration of the Real machine, ZSim and Sniper.}}
    \label{tablehb}
...
\section{\new{Investigating Current Architecture Prefetchers}}\label{sec:real_ipc}
...
\begin{figure}[!htb]
    ...
    \caption{\new{IPC results for the real execution of the NPB applications with input class A.}}
    \label{fig:ipc}
...
\begin{figure}[!htb]
    ...
    \caption{\new{Number of prefetch requests of the real machine executions with the input class A.}}
...
\begin{figure}[!htb]
    ...
    \caption{\new{CG communication pattern for 32 threads. Figure obtained from: Cruz \textit{et al.}~\cite{cruz2018thread}}}
    \label{fig:figcomma}
\end{figure}
...
\new{In Figure~\ref{fig:sniper_l2}}, we compare the performance, \new{in terms of IPC,} of the Sniper's L2 prefetcher with the L2 prefetcher of the real machine, and the Sniper's L1+L2 prefetcher with the real counterpart, respectively.
...
\new{In Figure~\ref{fig:sniper_l2-rqsts-all-pf} we present the total number of prefetch requests performed by the Sniper's L1+L2 prefetcher, the Sniper's L2 prefetcher, in ratio of their real counterparts.} 
The number of prefetch requests for the L1+L2 prefetcher counts the prefetches on both L1 and L2 caches.
...
\begin{figure}
    ...
    \caption{\new{Useless prefeteches performed by the simulation and by the real hardware, in ratio of the total number of prefetches.}}
